...
1. 停止所有{{nebula.name}}服务。
  ...
  `install_path`代表{{nebula.name}}的安装目录。
...
1. 准备{{nebula.name}} {{nebula.release}} 版本的安装包并解压，可指定任一安装目录。

2. 在解压的{{nebula.release}} 版本包的目录下，用其`bin`目录中的新版二进制文件替换{{nebula.name}}安装路径下`bin`目录中的旧版二进制文件。
...
4. 在`<install_path>/ent_yueshu_3.6.0/etc/nebula-meta.conf`中配置`license_manager_url`参数，指定 LM 的路径。
...
## 升级 2.x（x ≥ 5）至 {{nebula.release}}

升级 2.5.0 及之后的企业版集群至{{nebula.release}}的步骤同上文**升级 3.x（x < 4）至 {{nebula.release}}**部分描述。

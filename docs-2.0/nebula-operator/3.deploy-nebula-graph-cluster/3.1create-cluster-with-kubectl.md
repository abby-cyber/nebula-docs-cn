...
用户可以通过配置 Zone 的方式，将 Storage Pods 均匀地分布在不同的 Zone 中，并且优先将查询发送到同一 Zone 内的存储节点。关于 Zone 的详细信息，请参见[管理 Zone](../../4.deployment-and-installation/5.zone.md)。
...
| `spec.metad.config.zone_list`  | -                    | Zone 的名称列表，由逗号分隔。例如：zone1,zone2,zone3。<br/>**一旦设置，区域名称无法更改。**                             |
...
!!! caution
    ...
    创建集群后，导入数据前，确保存储 Pod 均匀分布在各个 Zone 中（即 storaged 的`replica`的数量应与 Zone 的数量成倍数关系），否则，可能会导致数据分布不均衡。运行`SHOW ZONES`命令来检查存储 Pod 是否均匀分布在各个 Zone 中。有关 Zone 相关命令，请参阅[管理 Zone](../../4.deployment-and-installation/5.zone.md)。

### Zone 功能

NebulaGraph 利用 Zone 的功能来高效管理其分布式架构。每个区域代表了存储完整图空间数据的 Storage Pod 的逻辑分组。NebulaGraph 的图空间数据被切分为不同分片，并且这些分片的副本均匀分布在所有可用的 Zone 中。使用 Zone 可以显着降低区域间的网络流量成本并提高数据传输速度。此外，区域内的数据读取可以提高可用性，因为分区的副本均匀分布在不同的 Zone 之间。

### 配置 Zone

如要充分利用 Zone 功能，首先需要确定集群节点所在的实际 Zone。通常情况下，部署在云平台上的节点都带有各自 Zone 的标签。一旦获取了这些信息，可以通过在集群的配置文件中设置`spec.metad.config.zone_list`参数来进行配置。该参数是一个由逗号分隔的 Zone 名称列表，并且应与节点实际所在的 Zone 名称相匹配。例如，如果您的节点位于 az1、az2 和 az3 区域，您的配置应如下所示：
...
一旦 Storage Pod 和 Graph Pod 被分配到了各个区域，与其对应的 Zone 之间的映射信息将存储在名为`<cluster_name>-graphd|storaged-zone`的 configmap 中。这个映射在滚动更新和 Pod 重启期间有助于 Pod 的调度，确保服务返回到所需的原始 Zone 中。

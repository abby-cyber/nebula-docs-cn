...
用户可以配置`spec.console`以在集群中启动 Console 容器。有关详细信息，请参阅 [nebula-console](https://github.com/vesoft-inc/nebula-operator/blob/v{{operator.release}}/doc/user/nebula_console.md#nebula-console)。
...
- 仅客户端与 Graph 服务之间的通信加密
  ...
  !!! note
        ...
        因为 Operator 没有通过接口调用 Graph 服务的需求，`sslCerts`里不用设置`clientSecret`。
